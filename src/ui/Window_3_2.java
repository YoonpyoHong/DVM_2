...
public class Window_3_2 extends JFrame implements ActionListener{
	...
	JLabel loc = new JLabel("Location: (x,y)", SwingConstants.CENTER);
	JLabel distance = new JLabel("Distance: m",SwingConstants.CENTER);
	...
	public Window_3_2(String title) {
		super(title);
	...
	private void init() {
		...
		loc.setPreferredSize(new Dimension(width - 400, height - 430));
		loc.setOpaque(true);
		loc.setBorder(BorderFactory.createLineBorder(Color.decode("#cfd0d1"), 1));
		
		distance.setPreferredSize(new Dimension(width - 400, height - 430));
